...
#define FONT_FILENAME "/home/anubrata/sem/09/arial/arial.ttf"
#endif
FTGLfont *font;
...
void arrow(double x, double y, double x1, double y1)
{
	...
	circle(x-4*x1,y-4*y1,0.2);
	glLineWidth(3.0);

	glColor3d(0.0,1.0,0.0);
	glBegin(GL_LINE_LOOP);
	glVertex2f(x,y);
	glVertex2f(x-5*x1+y1,y-5*y1-x1);
	glVertex2f(x-4*x1,y-4*y1);
	glVertex2f(x-5*x1-y1,y-5*y1+x1);
...
void opengl(Crossing cross[100],int start,int mark,int num, double speed)
{
	djikarr djik[100];
	...
	for(;;){
		...
		if (path[vehicle_pathIterator + 0] != -1 && path[vehicle_pathIterator + 1] != -1) {
			...
			double m1=atan2(x1-x0,y1-y0)*180/3.14;/*slope of current road*/
			double m2=atan2(x2-x1,y2-y1)*180/3.14;/*slope of next road*/
			double turn=m2-m1;/*calculation of angle between two roads*/
			
			/*calculation for the turn direction*/
			if(turn<0) turn=360+turn;
	

			      if(turn>15.0 && turn <180 && x1!=cross[mark].pos.x)
				{
				snprintf(p,100,"Est.Time %.2lf Turn Right at %.2lf",rtime,dti);
				glColor3d(1.0,1.0,0.0);
				outtextxy(vehicle_x,vehicle_y,p,ang,0.012);
				}
				if(turn>180.0 && turn <345.0 && x1!=cross[mark].pos.x)
				{
				snprintf(p,100,"Est.Time %.2lf Turn Left at %.2lf",rtime,dti);
				glColor3d(1.0,1.0,0.0);
				outtextxy(vehicle_x,vehicle_y,p,ang,0.012);
				}
				if(turn>350 || turn<10 ||x1==cross[mark].pos.x)
				{
				snprintf(p,100,"Est.Time %.2lf Continue Straight for %.2lf",rtime,dti);
				glColor3d(1.0,1.0,0.0);
				outtextxy(vehicle_x,vehicle_y,p,ang,0.012);
				}
		...
		usleep(t * 1000); /* Wait for about t millisecond */
	}
...
int searchmap(Crossing intersection[100], Position coordinates[100],int num)
{
	...
	printf("Enter 1,2, or 3 to search by->\n1) Name\n2) ID\n3) Coordinates\n\n~~Input your choice~~=>");  //giving user options for selecting search parameters
	...
	if(crit!=1 && crit!=2 && crit!=3)
	{
        ...
        if(crit==1)
        {
                printf("\nInput the name of the location(alphabets only)-> ");
                scanf(" %[^\n]",looknm);
        ...
        if(crit==2)
        {
                ...
                printf("\nInput the id of the location(integers only)-> ");
                scanf(" %d",&lookid);
        ...
        if(crit==3)
        {
                ...
                printf("\nInput the coordinates of the location  with a space in between(real numbers only)-> ");
...
void main()
{
	...
	printf("\n~~~Welcome to Takano senpai maps~~~\n\n");
	printf("Instructions of use:-\n\nPress Q and E to rotate the map\nPress W,A,S,D to pan\nPress Z and X to zoom in or out\nPress R to re-center\nPress T to view map in compass mode\n\n\n");
	...
	double speed;
	printf("\n\n\nHow fast do you wanna go?(Max speed>300, Min speed>100)--> ");
	scanf(" %lf",&speed);
	opengl(intersection,start,mark,num,speed);
